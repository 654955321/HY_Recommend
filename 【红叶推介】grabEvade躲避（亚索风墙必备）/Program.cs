...
namespace grabEvade
{
    class Program
    {
        ...
        private static Obj_AI_Hero champion = ObjectManager.Player;
        ...
        private static void OnGameLoad(EventArgs argv)
        {
            ...
            {
                generalMenu = new Menu("【红叶推介】grabEvade躲避"+scriptName, "general", true);
        ...
        private static void OnGameUpdate(EventArgs argv)
        {
            if (generalMenu.Item("enable").GetValue<bool>())
            {
                foreach (var shot in DetectedSkillshots)
                {
                    ...
                    {
                        foreach (Obj_AI_Hero champ in champion.GetAlliesInRange(1000))
                        {
