...
namespace LickyLicky
{
    class Program
    {
        static Menu mainMenu = new Menu("【红叶推介】Licky 河流之王", "Tahm", true);
        ...
        static void OnUpdate(EventArgs args)
        {
            ...
            {
                Obj_AI_Hero target =
                    ObjectManager.Get<Obj_AI_Hero>().Where(x => x.Distance(Player) <= Q.Range && x.IsEnemy && x.IsTargetable && !x.IsDead && x.IsChampion())
                ...
                if (target != null)
                {
                    ...
                    switch (buffCount)
                    {
                        ...
                        case 2:
                            ...
                            else if (mainMenu.SubMenu("Combo").Item("Use W").IsActive() && target.Distance(Player) > 350 && buffCount<2)
                            {
                                if (current == SwallowedTarget.None)
                                {
                                    ...
                                    W2.CastIfHitchanceEquals(target, HitChance.Medium);
                            }
                            break;
                        case 3:
                            if(mainMenu.SubMenu("Combo").Item("Use Q").IsActive())
                            ...
                            if (current == SwallowedTarget.None && mainMenu.SubMenu("Combo").Item("Use W").IsActive() && target.Distance(Player)<=250)
                                W.CastOnUnit(target);
                            break;
                    }
                }
                ...
                if (Player.ManaPercent >= mainMenu.SubMenu("Harass").Item("Harass Mana Percent").GetValue<Slider>().Value)
                {
                    ...
                    if (target != null)
                    {
                        ...
                        if (target.Distance(Player) <= 250 && target.GetBuffCount(buffName) == 3 && current == SwallowedTarget.None && mainMenu.SubMenu("Harass").Item("Use W").IsActive())
                            W.CastOnUnit(target);
                        else if (current == SwallowedTarget.None && mainMenu.SubMenu("Harass").Item("Use W").IsActive() && target.Distance(Player) > 350)
                        {
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            if (mainMenu.SubMenu("Drawing").Item("Draw Max W Movement").IsActive() && current == SwallowedTarget.None)
                Render.Circle.DrawCircle(Player.Position, ((Player.MoveSpeed * .05f *.5f) +110) * GetWLength(), System.Drawing.Color.MediumSpringGreen);
            else if(mainMenu.SubMenu("Drawing").Item("Draw Max W Movement").IsActive() && current == SwallowedTarget.Enemy)
                Render.Circle.DrawCircle(Player.Position, (Player.MoveSpeed * BuffDurationLeft(Player.GetBuff(tahmSlow))), System.Drawing.Color.MediumSpringGreen);
        }
        ...
        private static float GetWLength()
        {
            return ((W.Level - 1) * .25f) + 2;
        }
    }
}