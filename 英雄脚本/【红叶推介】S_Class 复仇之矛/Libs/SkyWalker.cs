...
namespace S_Class_Kalista
{
    public static class SkyWalker
    {
        ...
        public static void MoveTo(Vector3 position,
            float HoldZoneRadius = 0,
            bool overrideTimer = false,
            bool useFixedDistance = true,
            bool randomizeMinDistance = true)
        {
            if (Utils.GameTimeTickCount - LastMoveCommandT < _delay + _random.Next(0, 20) && !overrideTimer)
            {
                return;
            }
            ...
            if (playerPosition.Distance(position, true) < HoldZoneRadius * HoldZoneRadius)
            {
            ...
            bool randomizeMinDistance = true)
        {
            try
            {
                ...
                {
                    MoveTo(position, HoldZoneRadius, false, useFixedDistance, randomizeMinDistance);
                }
            }
            catch (Exception e)
            {
        ...
        public class Orbwalker
        {
            ...
            public Orbwalker(Menu attachToMenu)
            {
                ...
                drawings.AddItem(
                    new MenuItem("PlayerAutoCircle", "Player AA Circle").SetShared()
                          .SetValue(new Circle(true, Color.ForestGreen)));

                drawings.AddItem(
                    new MenuItem("EnemyAutoCircle", "Enemy AA circle").SetShared()
                        .SetValue(new Circle(false, Color.Firebrick)));

                drawings.AddItem(
                  new MenuItem("TargetCircle", "Target AA circle")
                      .SetValue(new Circle(true, Color.DarkOrange)));

                drawings.AddItem(
                    new MenuItem("HoldZone", "Hold Area").SetShared()
                        .SetValue(new Circle(false, Color.DodgerBlue)));

                drawings.AddItem(
                    new MenuItem("CircleWidth", "Line Width")).SetShared()
            ...
            public virtual AttackableUnit GetTarget()
            {
                ...
                if (ActiveMode == OrbwalkingMode.LaneClear || ActiveMode == OrbwalkingMode.Mixed ||
                    ActiveMode == OrbwalkingMode.LastHit || (Properties.MainMenu.Item("bUseMinionComboWalk").GetValue<bool>()))
                {
                    var MinionList =
                        ObjectManager.Get<Obj_AI_Minion>()
                            .Where(
                ...
                /*Lane Clear minions*/
                if (ActiveMode == OrbwalkingMode.LaneClear || (Properties.MainMenu.Item("bUseMinionComboWalk").GetValue<bool>() && ActiveMode == OrbwalkingMode.Combo))
                {
                    if (!ShouldWait())
                    {
            ...
            {
                if (!_config.Item("EnemyAutoCircle").GetValue<Circle>().Active
                  && _config.Item("TargetCircle").GetValue<Circle>().Active)
                {
                    ...
                    if (myTarget != null)
                    {
                        Render.Circle.DrawCircle(myTarget.Position, myTarget.IsValid<Obj_AI_Hero>() ? GetRealAutoAttackRange(myTarget) : 75,
                            _config.Item("TargetCircle").GetValue<Circle>().Color);
                    }
                }

                if (_config.Item("PlayerAutoCircle").GetValue<Circle>().Active)
                {
                    Render.Circle.DrawCircle(
                        Player.Position, GetRealAutoAttackRange(null) + 65,
                        _config.Item("PlayerAutoCircle").GetValue<Circle>().Color,
                        _config.Item("CircleWidth").GetValue<Slider>().Value);
                }
                ...
                if (_config.Item("EnemyAutoCircle").GetValue<Circle>().Active)
                {
                        ...
                        HeroManager.Enemies.FindAll(target => target.IsValidTarget(1175)))
                    {
                        Render.Circle.DrawCircle(
                            ...
                            _config.Item("EnemyAutoCircle").GetValue<Circle>().Color,
                            _config.Item("CircleWidth").GetValue<Slider>().Value);
                    }
                }
                ...
                if (_config.Item("HoldZone").GetValue<Circle>().Active)
                {
                    Render.Circle.DrawCircle(
                        ...
                        _config.Item("CircleWidth").GetValue<Slider>().Value, true);
                }
