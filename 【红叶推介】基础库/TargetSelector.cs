...
namespace LeagueSharp.Common
{
    public class TargetSelector
    {
        ...
        public static void AddToMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("ForceFocusSelected", "仅攻击被选择的目标").SetShared().SetValue(false)).Permashow();
            config.AddItem(
                new MenuItem("SelTColor", "被选择目标地下线圈颜色").SetShared().SetValue(new Circle(true, Color.Red)));
            config.AddItem(new MenuItem("Sep", "").SetShared());
            config.AddItem(new MenuItem("FuckSoraka", "对方有毒奶自动排列").SetShared().SetValue(true));
            ...
            config.AddItem(new MenuItem("Flowers", "红叶推介库"));
            config.AddItem(new MenuItem("Flowers1", "唯一QQ群:468103641"));
        }
