...
namespace xAwareness
{
    ...
    class ExtendedAwareness
    {
        ...
        private void Game_OnGameLoad(EventArgs args)
        {
            ...
            Menu positionAwarenessMenu = new Menu("【红叶推介】Mayomie 神意识", "xAwareness", true);
            positionAwarenessMenu.AddItem(new MenuItem("ShowAllyPosition", "显示 友方").SetValue(false));
            positionAwarenessMenu.AddItem(new MenuItem("ShowEnemyPosition", "显示 敌人").SetValue(true));
            //positionAwarenessMenu.AddItem(new MenuItem("ShowFowPosition", "Show Fow Position").SetValue(true));

            Menu rangeMenu = new Menu("范围设置", "RangeSettings");
            rangeMenu.AddItem(new MenuItem("PositionAwarenessRange",
                "极限 意识范围").SetValue(new Slider(6000, 0, 20000)));
            rangeMenu.AddItem(new MenuItem("InScreenAwarenessRange",
                "屏幕内 意识范围").SetValue(new Slider(0, 0, 250)));
            positionAwarenessMenu.AddSubMenu(rangeMenu);

            Menu iconMenu = new Menu("图标设置", "IconSettings");
            iconMenu.AddItem(new MenuItem("ShowPathDirection", "显示 路径方向").SetValue(true));
            iconMenu.AddItem(new MenuItem("ShowTeamColor", "显示 团队颜色").SetValue(true));
            iconMenu.AddItem(new MenuItem("IconOpacity",
                "图标 透明度").SetValue(new Slider(50, 0, 100)));
            iconMenu.AddItem(new MenuItem("IconScale",
                "图标 尺寸").SetValue(new Slider(35, 0, 100)));
            iconMenu.AddItem(new MenuItem("MinIconScale",
                "最小 图标尺寸").SetValue(new Slider(40, 0, 100)));

            iconMenu.AddItem(new MenuItem("IconSettingsDescription", "        --设置生效需要重载--"));
			iconMenu.AddItem(new MenuItem("hanhuabyergou", "           --Ergou汉化--"));
